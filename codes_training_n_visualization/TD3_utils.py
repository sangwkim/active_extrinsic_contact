...
class TD3:
    ...
    def compute_Q(self, state_full, action):
        
        state_full = np.tile(state_full, (action.shape[0],1,1))        
        state_full = torch.from_numpy(state_full).type(torch.FloatTensor).to(device)
        action = torch.from_numpy(action).type(torch.FloatTensor).unsqueeze(1).to(device)
        Q1 = self.critic_1(state_full/self.max_action, action/self.max_action)
        Q2 = self.critic_2(state_full/self.max_action, action/self.max_action)
    ...
    def save(self, directory, name):
        torch.save(self.actor.state_dict(),
                   '%s/%s_actor.pth' % (directory, name), _use_new_zipfile_serialization=False)
        torch.save(self.actor_target.state_dict(),
                   '%s/%s_actor_target.pth' % (directory, name), _use_new_zipfile_serialization=False)

        torch.save(self.critic_1.state_dict(),
                   '%s/%s_crtic_1.pth' % (directory, name), _use_new_zipfile_serialization=False)
        torch.save(self.critic_1_target.state_dict(),
                   '%s/%s_critic_1_target.pth' % (directory, name), _use_new_zipfile_serialization=False)

        torch.save(self.critic_2.state_dict(),
                   '%s/%s_crtic_2.pth' % (directory, name), _use_new_zipfile_serialization=False)
        torch.save(self.critic_2_target.state_dict(),
                   '%s/%s_critic_2_target.pth' % (directory, name), _use_new_zipfile_serialization=False)
