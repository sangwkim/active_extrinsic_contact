...
class PhysicsFactor_v2_2: public gtsam::NoiseModelFactor4<gtsam::Pose2, gtsam::Pose2, gtsam::Point2, gtsam::Vector1> {
...
public:
  ...
  PhysicsFactor_v2_2(gtsam::Key key1, gtsam::Key key2, gtsam::Key key3, gtsam::Key key4,
  ...
  gtsam::Vector evaluateError(const gtsam::Pose2& p1, const gtsam::Pose2& p2,
    ...
    gtsam::Matrix Hr1 = (gtsam::Matrix13() << 0, -1, 0).finished();
    gtsam::Matrix Hr2 = (gtsam::Matrix13() << 0, 0, -std::sin(p2.theta())*(p3[1]-p1.y()) - std::cos(p2.theta())*(p3[0]-p1.x())).finished();
    
    if (H1) *H1 = (gtsam::Matrix21() << 1/p4[0], 1/p4[0]).finished() * Hr1
                    + (gtsam::Matrix23() << 0, 0,  mtheta_/mx_*std::sin(p2.theta()-p1.theta()),
                                            0, 0, -mtheta_/my_*std::cos(p2.theta()-p1.theta())).finished();
    if (H2) *H2 = (gtsam::Matrix21() << 1/p4[0], 1/p4[0]).finished() * Hr2
                    + (gtsam::Matrix23() << 0, 0, -mtheta_/mx_*std::sin(p2.theta()-p1.theta()),
                                            0, 0,  mtheta_/my_*std::cos(p2.theta()-p1.theta())).finished();
    if (H3) *H3 = (gtsam::Matrix21() << 1/p4[0], 1/p4[0]).finished() * (gtsam::Matrix12() << -std::sin(p2.theta()), std::cos(p2.theta())).finished();
    if (H4) *H4 = (gtsam::Matrix21() << -r / std::pow(p4[0],2), -r / std::pow(p4[0],2)).finished();

    return (gtsam::Vector2() << (r/p4[0]) + (mtheta_/mx_*std::cos(p2.theta()-p1.theta())),
                                (r/p4[0]) + (mtheta_/my_*std::sin(p2.theta()-p1.theta()))).finished();
  }
